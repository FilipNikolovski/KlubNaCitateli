            ...
            using (MySqlConnection connection = new MySqlConnection())
            {
                try
                {
                   ...
                   // ----------------------------------------------
                    ...
                    //Ako ima najaven korisnik--------------------------------------------
                    if (Session["Id"] != null)
                    {
                        List<string> namesCategories = new List<string>();
                        //Prvo gi zema kategoriite na najaveniot korisnik---------------------------------------
                        ...
                        reader.Close();
                        //Ako nema izbrano 6 kategorii dopolni so random kategorii
                        ...
                        //ako nema najaven korisnik------------------------------------
                    else
                    {
                        List<string> namesCategories = new List<string>();
                        //zemi 6 random kategorii so broj na knigi pogolem od 0
                        ...
                        reader.Close();
                        //Ako brojot na kategorii so broj na knigi pogolem od 0 e pomal od 6 dopolni do 6 so random kategorii od tie so najmalku edna kniga
                        while (categoriesList.Count <= 6)
                        {
                            command.CommandText = "select categories.idcategory, name from categories, belongsto where belongsto.idcategory=categories.idcategory group by idcategory having count(idbook) > 0 order by rand() limit 1";
                            command.Connection = connection;
